...
export function IndexesSection({ jumpTo, onCompare }: { jumpTo?: string | null; onCompare?: (symbol: string) => void }) {
  ...
  const vols = useVolatility(INDEXES.map(i => i.symbol));
  ...
  // shows even when a tool is active (the dual-line chart only renders with no tool active).
  ...
  const filtered = INDEXES.filter(i => {
    ...
    return (selectedRegion === 'All' || i.region === selectedRegion)
      ...
      && (volFilter === 'all' || volBand(vols.get(i.symbol)?.total) === volFilter);
  });
              ...
              <button key={idx.symbol}
                ...
                {q && q.price > 0 ? (
                  <>
                    ...
                    {/* Period returns in two columns to keep the card compact */}
                    ...
                    <VolatilityLine vol={vols.get(idx.symbol)} />
                  </>
                ) : (
