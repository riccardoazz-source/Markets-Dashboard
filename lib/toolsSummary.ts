import { EMA_WEEKS, type ActiveTools } from '@/components/ui/ChartTools';
import { HistoricalPoint } from './types';
import {
  computeSMA, computeEMA, computeRSI, computeMACD, computeBollingerBands, computeMomentum,
  computeSma200wLatest, computeEmaWeeklyLatest, computeRsiResampledDaily, computeMacdResampledDaily, computeTrendLine,
...
export function summarizeTools(t: ActiveTools, points: HistoricalPoint[]): string[] {
  ...
  if (t.emaWeekly) withVal(`EMA ${EMA_WEEKS}W`, computeEmaWeeklyLatest(dates, rawCloses, EMA_WEEKS));
