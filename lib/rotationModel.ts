...
export interface ModelInput {
  ...
  high52w?: number | null;  // 52-week high (kept for callers; not used by EMS v3)
...
export function computeRotationFeatures<T extends ModelInput>(items: T[]): RotationFeature<T>[] {
  ...
  // ── M31 v3 Dalio EMS inputs (param-independent cross-sectional percentiles) ──
  // Momentum blend = 0.4·pctile(Ret20) + 0.3·pctile(Ret60) + 0.3·pctile(Ret12m)
  // (long-horizon-weighted). Persistence = pctile(Ret20/Ret60) — ACCELERATION.
  // r1m (~21 td) is the Ret20 fallback; Ret60≈3-month (r3m), Ret12m≈1-year (r1y).
  ...
  const s12 = dSortAsc(i => i.r1y ?? null);
  const dalioAccelOf = (i: ModelInput): number | null => {
    const r20 = dalioRetOf(i);
    return r20 != null && i.r3m != null && i.r3m > 0 ? r20 / i.r3m : null;
  };
  const sAccel = dSortAsc(dalioAccelOf);
  ...
  return items.map(item => {
    ...
    const p12 = item.r1y != null ? pctIn(s12, item.r1y) : 0.5;
    const dalioM = DALIO_MBLEND_W[0] * p20 + DALIO_MBLEND_W[1] * p60 + DALIO_MBLEND_W[2] * p12;
...
export function scoreFromFeatures<T extends ModelInput>(
  ...
  return features.map(f => {
    ...
    if (MODEL_MODE === 'dalio') {
      ...
      // (v7/v8 recovery sleeve retired — see selectPicks. dalioRecoveryScore/downVolDry
      // stay in the codebase, unused, to document the experiment.)
      return {
...
export function selectPicks<T extends ModelInput>(
  ...
  // M31 Dalio selection: global top-N by EMS score. The reserved-slot recovery
  // sleeve (v7 no-gate, v8 prove-it-first gate) was RETIRED: Ray confirmed it is an
  // information ceiling, not a tuning problem — at a new low a future winner and a
  // future loser are indistinguishable from price/volume alone, so the sleeve either
  // buys knives (v7 → reliability 12.9) or misses the not-yet-turned winners (v8 →
  // 24.5), never above the no-sleeve v6 (25.6). Restored to v6.
  if (MODEL_MODE === 'dalio') {
    return scored
