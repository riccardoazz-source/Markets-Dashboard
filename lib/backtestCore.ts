...
export function buildInputsAsOf(universe: BtMeta[], histMap: Map<string, Hist>, asOfDate: Date): BtInput[] {
  ...
  return universe.map(m => {
    ...
    // Quadrant coordinates: how deep this cycle has gone and how far into its current
    // leg the price has travelled. Computed on the FULL history `h` (not the 400-bar
    // tail) because the cycle is a running state that has to be walked from the
    ...
    return {
      ...
      downVolDry: downVolDryUp(closesAsOf, upToAsOf.map(p => p.volume)),
