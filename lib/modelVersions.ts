...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  },
  {
    ...
    formula: [
      ...
      '',
      'THE v3 → v4 FIX: v3 collapsed to reliability 0.0 (it bought world indices and MISSED every',
      'individual winner — captured 0/9 at 1Y, 0/14 at 3M). Cause: the two HARD gates DistMA ≤ 0.15',
      'and R2_12m > 0.5 structurally excluded high-beta stocks (they run far above their MA with a',
      'spikier, lower-R² path) and admitted only smooth low-vol indices. Ray SOFTENED both guards:',
      '',
      'decay (replaces the DistMA hard cap — Ray\'s #1 fix): bites ONLY when the move is far above the',
      '   MA AND a short-term blow-off:  decay = exp(−0.7·max(0, DistMA − 0.15))  when Ret20 > 30%, else 1.',
      '   A stock 40% above its MA with modest recent gains keeps a full score; one 40% above AND up',
      '   35% in a month is damped. No more wholesale exclusion of strong trends.',
      'TrendQuality (replaces the R² hard gate): a SOFT additive reward: min(1, R2_12m/0.8) × weight 0.15.',
      '   Clean trends get a boost, but a volatile winner is no longer disqualified.',
      '',
      'V (flow): volume → max(0, VolRatio − 1.1); volume-blind → 0.20·RangeExpansion.',
      'M (momentum blend): 0.4·pctile(Ret20) + 0.3·pctile(Ret60) + 0.3·pctile(Ret12m).',
      'Persistence = pctile(Ret20 ÷ Ret60) — acceleration.',
      'Overheat (commodity brake): Price/Median12m > 1.5 AND VolRatio > 1.3 → Score ×= (1 − 0.5·Overheat).',
      'ExitFactor (exhaustion): 1 if Ret20 ≥ Ret60 else 0.7; ×0.8 more if R2_12m < 0.5.',
      '',
      'NO hard cycle gate any more — ranking is GLOBAL by Score (top 25), tie-break VolRatio then RelStr.',
      'The high-return winners rise to the top on M, which is what the capture objective needs. (Ray also',
      'proposed per-asset-class slots for portfolio diversification; deliberately NOT applied — capping',
      'stock slots would cap the very stock-capture we optimise for. Trivial to add if wanted.)',
      '',
      'Selectable like M26/M27: MODEL_MODE = \'dalio\'; flip to \'rotation\' to restore M24 exactly.',
