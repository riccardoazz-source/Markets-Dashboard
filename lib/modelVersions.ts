...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  {
    id: 5,
    name: 'Group diversity caps — break correlated ETF clusters',
    current: true,
    formula: [
      'Score = 0.38·ACC + 0.22·TRD + 0.12·LEAD + 0.10·REG + 0.04·VOL − wEXT·EXT',
      '   (weights unchanged from M4 — the key change is in SELECTION, not scoring)',
      '',
      'NEW: per-group pick caps applied at selection time:',
      '   Sectors  → max 3 picks   (thematic ETFs cluster, all peak and crash together)',
      '   Indexes  → max 3 picks   (broad indexes are correlated too)',
      '   Crypto   → max 3 picks',
      '   Commodities → max 2 picks   (already penalised by heavy EXT weight 0.32)',
      '   Stocks   → no cap   (individual names are far less correlated)',
      '',
      'WHY: at peak-momentum moments the old model filled 5-6 slots with correlated',
      '   thematic ETFs (WCLD + CIBR + AIQ + XLK…) that all peaked in Jun 2021 and',
      '   all crashed together. Individual winners (NVDA, PLTR, MU) were buried below',
      '   the cluster even when they outscored most of it. The cap breaks the cluster:',
      '   once 3 Sectors are chosen, the 4th slot goes to the next best across ALL',
      '   other groups, forcing real diversification.',
      '',
      'Selection = selectWithGroupCap(scored, 20) — greedy descent by score, skip if',
      '   the group cap is already full, continue until 20 total or universe exhausted.',
      '',
