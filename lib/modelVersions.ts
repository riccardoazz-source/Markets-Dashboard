...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  {
    id: 31,
    name: 'Dalio EMS v2 — flow × multi-horizon momentum × trend-quality, sustainability cycle gate',
    current: true,
    formula: [
      'EMS = C · (0.40·V + 0.40·M_final + 0.20·Persistence)   — Ray\'s refined transaction-flow model',
      '',
      'V (flow / transaction size):',
      '   volume assets → max(0, VolRatio − 1.1)   VolRatio = 5d ADV ÷ 60d ADV, 5-day smoothed',
      '   volume-blind  → 0.20 · RangeExpansion    (indices/futures/foreign listings keep the flow',
      '     thesis via a close-only daily-range-expansion proxy, scaled to the same 0–0.2 band)',
      '',
      'M_final (multi-horizon, confirmed, blow-off-penalised — fixes the single-window whipsaw):',
      '   M_blend  = 0.5·pctile(Ret20) + 0.3·pctile(Ret60) + 0.2·pctile(Ret180)   (20d / 3M / 6M)',
      '   Confirmed = Price > MA200 AND Ret60 > 0        (else M_final = 0)',
      '   BlowOff   = Ret20 > 30% AND VolRatio < 1.2 → penalty 0.5·(Ret20−0.30)/(1−0.30)',
      '   M_final   = M_blend · Confirmed · (1 − BlowOff)',
      '',
      'Persistence = pctile(Ret60 ÷ Ret20)  for Ret20 > 0 — trend-quality: a short-term pop that is',
      '   part of a longer up-trend (durable compounder) outranks an isolated one-month spike.',
      '',
      ...
      '       else 0.   (The old v1 gate let almost every laggard through → collapsed onto momentum.)',
      '',
      ...
      '',
      'Selectable like M26/M27: MODEL_MODE = \'dalio\' in lib/rotationModel.ts; flip to \'rotation\' to',
      'restore M24 exactly. Backtest feeds real historical volume + range-expansion into the as-of',
      'inputs (rvol5 / high52w / r20 / rangeExp), no look-ahead. SeasonFactor deferred. Ret60≈3M,',
      'Ret180≈6M. ⭐ macro pins display-only. (Weights 0.40/0.40/0.20 & the range proxy are my',
      'reconciliations — Ray\'s final consolidated block arrived truncated.)',
    ],
