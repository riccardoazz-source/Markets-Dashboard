...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  {
    id: 31,
    name: 'Dalio EMS v7 — deep-value/recovery sleeve (6 reserved slots, max-recall): catch the falling winners a momentum score can\'t rank',
    current: true,
    formula: [
      'FinalScore = (core + 0.10·AccelBoost + 0.30·DrawdownQuality) · ClassWeight',
      '   core = decay·(0.45·V + 0.45·M + 0.10·Persistence)·(1 − 0.5·Overheat)·ExitFactor + 0.15·TrendQuality',
      '',
      'THE v4 → v5 FIX: v4 fixed the 5Y (from reliability 0.0 to +140.7% vs S&P at 5Y, capture 8/25) but',
      'overall reliability was still 27.0 (< M24\'s 34.7). The gap: the biggest 1Y winners (SNDK +4056%,',
      'MU +686%, WULF +349%, AMD +281%, LRCX) were FALLING/basing at the pick date — a pure momentum',
      'model can\'t buy a falling asset. Ray added two overlays + a class tilt:',
      '',
      'AccelBoost (Ray\'s #1 change): accel = Ret5/Ret20; boost = clamp(accel − 1, 0, 1) when Ret20 > 0.',
      '   Weight 0.10. Catches a name turning up faster than its own 20-day trend — the early breakout.',
      'DrawdownQuality (the falling-winner sleeve, weight 0.30) — v6 has THREE tiers (Ray, to lift capture of',
      '   the deepest winners like SNDK/MU/WULF/AMD that v5 still missed):',
      '   • SHALLOW (−35%..+5% of MA), quality name (Ret12m > 0): VolRatio ≥ 1.1 + net buying → full weight.',
      '   • DEEP (−60%..−35%): the STRICT transaction signature only — VolRatio ≥ 1.5 AND moneyFlow > 0.1',
      '     (≈ Up/Down-Vol-Ratio > 1.2 and OBV slope up), NO Ret12m gate, capped at half weight. Separates a',
      '     base from a knife by flow, not price.',
      '   • THIN history (no Ret12m — spin-offs/IPOs): short-term uptrend (Ret20 > 0 AND Ret5 > 0) + volume',
      '     accumulation, 0.6× weight. moneyFlow is the 20-day net directional volume (doubles as OBV/UDR).',
      'ClassWeight (Ray\'s diversification-without-slots): stocks ×1.2, sectors ×1.0, crypto ×1.1, commodities',
      '   ×0.9, indices ×0.8 — a light edge to the high-beta leaders that actually win.',
      '',
      'v7 — DEEP-VALUE/RECOVERY SLEEVE (Ray\'s max-recall lever): v6\'s accumulation gates required POSITIVE',
      '   money flow, so the biggest winners (SNDK, MU, WULF, AMD, LRCX) — which were FALLING with net',
      '   SELLING at the pick date — were still skipped (capture stuck at 42/150). Per the user\'s goal of',
      '   MAX capture, v7 reserves 6 of the 25 slots for high-beta names in a real drawdown (≥10% below the',
      '   MA, not utterly broken) that HAD a structural trend (12-month R² > 0.3 or Ret12m > 0), ranked by',
      '   drawdown depth × prior-trend strength. NO positive-money-flow gate — only a soft "not in freefall"',
      '   floor (moneyFlow ≥ −0.35) keeps out pure knives. Trades precision for recall (like M24\'s sleeve,',
      '   which caught 51/150 vs our 42). selectPicks fills these 6 first, then the top 19 by EMS score.',
      '',
      'v4 base kept: decay soft blow-off brake (exp(−0.7·max(0,DistMA−0.15)) only when Ret20 > 30%),',
      'TrendQuality soft R² reward (min(1,R²/0.8)·0.15), V flow, M = 0.4·Ret20+0.3·Ret60+0.3·Ret12m,',
      'Persistence = pctile(Ret20/Ret60), commodity Overheat, exhaustion ExitFactor. No hard cycle gate;',
      'ranking is GLOBAL by score (top 25), tie-break VolRatio then RelStr.',
      '',
      'Selectable like M26/M27: MODEL_MODE = \'dalio\'; flip to \'rotation\' to restore M24 exactly. Inputs',
      '(as-of, no look-ahead): rvol5 / r5 / r20 / r3m / r1y / rangeExp / median12m / moneyFlow; R2_12m = trendR2Long.',
    ],
