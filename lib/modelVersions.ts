...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  {
    id: 31,
    name: 'Dalio EMS v3 — consolidated formula: capped cycle gate, R²-clean trend, commodity overheat + exit brakes',
    current: true,
    formula: [
      'FinalScore = C · (0.45·V + 0.45·M + 0.10·Persistence) · (1 − 0.5·Overheat) · ExitFactor',
      '',
      'THE v2 → v3 FIX (v2 barely beat S&P at 5Y — reliability 27.0, below M24\'s 34.7 — because it',
      'bought the Jul-2021 commodity/software TOPS: WTI, Brent, Nat Gas, Corn, ADBE, INTU, all in',
      '6-month uptrends above their MA200 that then mean-reverted 20–63%). Ray\'s consolidated fix:',
      '',
      'Cycle gate C — HARD distance cap + clean-trend requirement (kills late-cycle blow-offs):',
      '   C = 1 if DistMA ≤ 0.15 AND Ret60 > 0 AND R2_12m > 0.5, else 0',
      '     DistMA = Price/MA200 − 1  → ≤15% above the MA (no extended parabola)',
      '     Ret60 ≈ 3-month return > 0',
      '     R2_12m = R² of the 12-month price trend > 0.5 → a CLEAN trend, not a spiky commodity pop',
      '',
      'V (flow): volume → max(0, VolRatio − 1.1); volume-blind → 0.20·RangeExpansion.',
      'M (momentum blend, now long-horizon-weighted): 0.4·pctile(Ret20) + 0.3·pctile(Ret60) + 0.3·pctile(Ret12m).',
      'Persistence = pctile(Ret20 ÷ Ret60) — ACCELERATION (recent pace ≥ older pace; v2\'s inverse',
      '   rewarded deceleration — Ray flipped it).',
      '',
      'Overheat (commodity late-cycle brake): for commodities, Price/Median12m > 1.5 AND VolRatio > 1.3 →',
      '   Overheat = clamp(min(1,(ratio−1.5)/0.5)·(VolRatio−1.3), 0, 1); Score ×= (1 − 0.5·Overheat).',
      'ExitFactor (exhaustion brake): 1 if Ret20 ≥ Ret60 else 0.7; ×0.8 more if R2_12m < 0.5.',
      '',
      'Rank: FinalScore desc · tie-break VolRatio desc, then RelStr desc. (v3 drops the hard RelStr>0',
      '   pre-filter — RelStr is a tie-break only now, per Ray\'s consolidated formula.)',
      'Picks: every qualifying name up to 25 — no sleeve, no other pillars.',
      '',
      'Selectable like M26/M27: MODEL_MODE = \'dalio\'; flip to \'rotation\' to restore M24 exactly.',
      'Backtest feeds real historical volume + range-expansion + 12-month median into the as-of inputs',
      '(rvol5 / r20 / rangeExp / median12m; R2_12m = trendR2Long), no look-ahead. Ret60≈3M, Ret12m≈1Y.',
      '⭐ macro pins display-only. (Ray\'s final block was complete this time — implemented verbatim.)',
    ],
