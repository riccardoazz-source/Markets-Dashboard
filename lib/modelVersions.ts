...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  {
    id: 36,
    name: 'Dalio EMS v8 — precise recovery sleeve: prove-it-first timing gate + down-vol dry-up composite (4 slots)  ← LIVE MODEL',
    current: true,
    formula: [
      'Fixes v7\'s "bought falling knives" problem with Ray\'s precision recipe for the recovery sleeve:',
      '',
      'TIMING GATE (the key — "prove-it-first"): a recovery candidate must have a POSITIVE 5-day return',
      '   (Ret5 > 0) — it is already TURNING UP, not still making new lows. This alone filters out the',
      '   "falling and kept falling" names that crashed v7.',
      'COMPOSITE RANK (Ray\'s weights) among names that clear the gate:',
      '   0.40 · down-day-volume DRY-UP  (recent 5-day down-day volume vs the prior 20 down-days — seller',
      '          exhaustion, the strongest single discriminator)',
      '   0.25 · drawdown DECELERATION   (recent daily pace better than the 20-day daily pace — the fall is fading)',
      '   0.20 · RELATIVE STRENGTH       (falling less than the ^GSPC benchmark)',
      '   0.15 · drawdown DEPTH          (deeper = more recovery upside)',
      'FEWER, higher-conviction slots: 6 → 4 reserved slots. Still gated to a real drawdown (≥10% below the MA,',
      '   > −70%) with a prior structural trend (R2_12m > 0.3 or Ret12m > 0).',
      '',
      'Everything else = v6 (accel overlay, 3-tier accumulation, class tilt, decay, trend-quality, overheat,',
      'exit). New as-of input: downVolDry (down-day volume dry-up 0..1). MODEL_MODE = \'dalio\'.',
    ],
