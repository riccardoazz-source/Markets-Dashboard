...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  },
  {
    ...
    results: {}, // frozen at last recorded live results
  },
  {
    id: 24,
    name: 'Auto-tuned weights via 10k-combination sweep (+6.7pp capture)',
    current: true,
    formula: [
      'Score = 0.15·ACC + 0.35·VQ + 0.00·TRD + 0.10·CYC + 0.07·LEAD + 0.09·REG',
      '        + 0.00·VOL + 0.00·MACD − wEXT·EXT − wOH·OH + REBOUND − LOWVQ',
      '',
      'ALL 18 tunable parameters auto-optimized via a 10,000-combination parameter sweep.',
      'Fitness = mean capture across ~40 historical dates (anti-overfit). Baseline: 27.5%.',
      'Best found: 34.2% (+6.7pp capture). Key findings:',
      '',
      '• ACC weight halved (0.34→0.15): raw acceleration is noisier than quality signals.',
      '• VQ raised (0.26→0.35): net upside volatility is the single strongest predictor.',
      '• TRD eliminated (0.08→0.00): fully captured by ACC·aRecent + LEAD·trendR2.',
      '• VOL eliminated (0.04→0.00): null→0.5 neutral in backtest; pure noise.',
      '• MACD near-zero (0.04→0.004): already embedded in acceleration signal.',
      '• REG raised (0.04→0.09): MA200 regime is more predictive than previously weighted.',
      '• REBOUND near-zero (0.30→0.017): across all dates the bonus hurts more than it helps;',
      '  the pre-breakout sleeve (8 slots) already catches falling quality engines.',
      '• cyclicalVqDiscount raised (0.50→0.962): cyclicals now treated like seculars for VQ.',
      '  M23\'s pCyc-based exemption already differentiates fear-spikes from secular trends.',
      '• overheatCyclical lighter (0.12→0.038): same reasoning; M23 exemption handles nuance.',
      '• lowVqWeight stronger (0.32→0.50): definitively sinks low-VQ defensives.',
      '• commodityExtWeight equalised (0.32→0.20): matches base EXT; secular exemption differs.',
      '• Secular fade window widened: LOW 0.55→0.458, HIGH 0.75→0.822.',
    ],
