...
export const MODEL_VERSIONS: ModelVersion[] = [
  ...
  // ── M31 = the Dalio "EMS" line (a DISTINCT philosophy from M1–M30, selectable via
  // MODEL_MODE='dalio'). Each iteration below is FROZEN as its own row so the whole
  // lineage is comparable in the table. Results are read off the live backtest cards
  // at each version's deploy (winnerHits per period estimated where a card wasn't
  // captured; headline reliability/capture are exact). v6 is the live model — v7's
  // recovery-sleeve experiment made it much worse and was reverted (see below). ──
  {
    id: 31,
    name: 'Dalio EMS v1–v3 — the transaction-flow build-up, ending in the v3 HARD-GATE collapse',
    current: false,
    recordedAt: '2026-07-12',
    formula: [
      'The Dalio "Early-Momentum Score" (EMS) — a transaction-flow philosophy (volume/flow × momentum ×',
      'cycle), NOT a remix of the M1–M30 pillars. Built iteratively with the source ("Ray"):',
      '',
      'v1: EMS = C·(0.45·V + 0.45·M) — V = max(0,VolRatio−1.1) (5d/60d ADV, volume-blind → 1.0 neutral),',
      '    M = pctile(positive Ret20), C = near-52w-high & ≤15% over MA, or >5% off the high w/o a spike.',
      '    Problem: C ≈ 1 for the whole universe → the score collapsed onto 1-month momentum.',
      'v2: multi-horizon M (0.5·20d+0.3·3M+0.2·6M) + confirmation (Price>MA200 & 3M>0) + blow-off penalty +',
      '    persistence; volume-blind got a range-expansion flow proxy. Still short-horizon-heavy.',
      'v3 (CONSOLIDATED, then it collapsed): FinalScore = C·(0.45·V+0.45·M+0.10·Persistence)·(1−0.5·Overheat)·ExitFactor',
      '    with a HARD cycle gate C = 1 iff DistMA ≤ 0.15 AND Ret60 > 0 AND R2_12m > 0.5, momentum blend',
      '    0.4/0.3/0.3 on 20d/3M/12M, persistence = pctile(Ret20/Ret60), commodity overheat + exhaustion exit.',
      '    THE FAILURE: the DistMA ≤ 0.15 cap + R² > 0.5 gate structurally excluded high-beta stocks (they',
      '    run far above their MA with a spikier, lower-R² path) and admitted only smooth low-vol INDICES.',
      '    The model became "buy world indices" → below the S&P at 5Y, captured 0/9 of the 1Y leaders.',
      '',
      'Inputs (as-of, no look-ahead): rvol5 / r20 / r3m / r1y / rangeExp / median12m; R2_12m = trendR2Long.',
    ],
    results: {
      // v3 live backtest (2026-07-12) — the index-only collapse. Fewer picks (strict gate).
      '3m': { basket:  21.4, spx: 10.0, picks: 20, winnerHits:  6, winnerTotal: 20 }, // estimated hits
      '6m': { basket:   8.1, spx:  8.7, picks: 17, winnerHits:  1, winnerTotal: 17 },
      '1y': { basket:  32.2, spx: 21.0, picks:  9, winnerHits:  0, winnerTotal:  9 },
      '5y': { basket:  72.6, spx: 73.4, picks: 24, winnerHits:  4, winnerTotal: 24 }, // 5Y < S&P → reliability 0.0
    },
  },
  {
    id: 32,
    name: 'Dalio EMS v4 — un-biased guards: soft blow-off decay + soft trend-quality (fixes the v3 index-only collapse)',
    current: false,
    recordedAt: '2026-07-12',
    formula: [
      'Score = decay·(0.45·V + 0.45·M + 0.10·Persistence)·(1 − 0.5·Overheat)·ExitFactor + 0.15·TrendQuality',
      '',
      'Ray SOFTENED the two v3 hard gates so high-beta winners survive:',
      'decay (replaces the DistMA cap): exp(−0.7·max(0, DistMA − 0.15)) ONLY when Ret20 > 30% — a blow-off',
      '   must be BOTH far above the MA AND a short-term surge; otherwise 1 (no penalty). Strong trends live.',
      'TrendQuality (replaces the R² gate): soft additive min(1, R²/0.8) × 0.15 — a volatile winner is no',
      '   longer disqualified. No hard cycle gate; ranking is GLOBAL by score (top 25).',
      '',
      'Result: fixed the 5Y disaster (+140.7% vs S&P +73.4%, capture 8/25) but reliability 27.0 < M24 34.7 —',
      'the biggest 1Y winners were still missed (they were falling at the pick date; a momentum score can\'t rank them).',
    ],
    results: {
      '1m': { basket:  -0.7, spx:  1.8, picks: 25, winnerHits: 10, winnerTotal: 25 }, // 1m/1d estimated
      '3m': { basket:  20.2, spx: 10.0, picks: 25, winnerHits: 10, winnerTotal: 25 },
      '6m': { basket:  10.0, spx:  8.7, picks: 25, winnerHits:  8, winnerTotal: 25 },
      '1y': { basket:  24.0, spx: 21.0, picks: 25, winnerHits:  7, winnerTotal: 25 },
      '5y': { basket: 140.7, spx: 73.4, picks: 25, winnerHits:  8, winnerTotal: 25 },
    },
  },
  {
    id: 33,
    name: 'Dalio EMS v5 — acceleration overlay + quiet-accumulation sleeve + class tilt (catch the falling winners)',
    current: false,
    recordedAt: '2026-07-12',
    formula: [
      'FinalScore = (core + 0.10·AccelBoost + 0.30·DrawdownQuality) · ClassWeight   (core = the v4 formula)',
      '',
      'AccelBoost (Ray\'s #1 change): clamp(Ret5/Ret20 − 1, 0, 1) when Ret20 > 0 — a name turning up faster',
      '   than its own 20-day trend (early breakout).',
      'DrawdownQuality (falling-winner sleeve): a quality name (Ret12m > 0, R²) at/below its MA (−35%..+5%)',
      '   with volume building AND positive money flow (up-vol − down-vol over 20d) — "buyers accumulating',
      '   before the crowd". 0 unless every condition aligns, so never a falling knife.',
      'ClassWeight: stocks ×1.2 … indices ×0.8 (diversification without hard slots).',
      '',
      'Result: TRANSFORMED the returns — beats the S&P at 5 of 6 horizons (5Y +171%, 1Y +61.8%, 6M +15.6%,',
      '3M +24.9%). But reliability stayed 25.8: the score is capture-primary + magnitude-blind, and capture',
      'held at ~8/25 (the huge return gains don\'t move it). New inputs: r5 (5-day return), moneyFlow.',
    ],
    results: {
      '1m': { basket:  -1.6, spx:  1.8, picks: 25, winnerHits:  6, winnerTotal: 25 }, // 1m/1d estimated
  ...
  },
  {
    ...
    formula: [
      ...
      '   • DEEP (−60%..−35%): STRICT signature only — VolRatio ≥ 1.5 AND moneyFlow > 0.1 (≈ Up/Down-Vol-Ratio',
      '     > 1.2, OBV slope up), NO Ret12m gate, half weight. Separates a base from a knife by FLOW, not price.',
      '   • THIN history (no Ret12m — spin-offs/IPOs): short-term uptrend (Ret20 > 0 AND Ret5 > 0) + volume',
      '     accumulation, 0.6× weight.',
      '',
      'Result: the deep/thin tiers did not measurably change the backtest (capture 42/150, reliability 25.8 —',
      'byte-identical to v5): the biggest missed winners were falling with NET SELLING at the pick date, so the',
      'positive-money-flow requirement still rejected them. This is the LIVE model. Its RETURNS beat M24 at 5Y',
  ...
  {
    id: 35,
    name: 'Dalio EMS v7 — recovery sleeve, first run (6 slots, no timing gate) — the sleeve bought falling losers',
    current: false,
    recordedAt: '2026-07-12',
    formula: [
      ...
      '',
      'OUTCOME: the sleeve filled with names that were falling AND kept falling — at the pick date a future',
      '+686% winner and a future −60% loser both look like "a high-beta stock in a drawdown", and there are far',
      'more falling losers than winners. Capture did not rise (40/150) and the losers cratered returns',
      '(1Y +61.8% → +14.3%, below the S&P) → reliability 25.8 → 12.9. FIX in v8: a "prove-it-first" timing gate.',
    ],
    results: {
      ...
      '1y': { basket:  14.3, spx: 21.0, picks: 25, winnerHits:  7, winnerTotal: 25 }, // 1Y < S&P
      '5y': { basket: 168.0, spx: 73.4, picks: 25, winnerHits:  8, winnerTotal: 25 },
    },
  },
  {
    id: 36,
    name: 'Dalio EMS v8 — precise recovery sleeve: prove-it-first timing gate + down-vol dry-up (4 slots) — RETIRED (info ceiling)',
    current: false,
    recordedAt: '2026-07-12',
    formula: [
      'v7\'s fix: a recovery candidate must clear a "prove-it-first" TIMING GATE (positive 5-day return — it is',
      'already turning up, not still making new lows), then rank by a composite (0.40·down-day-volume dry-up +',
      '0.25·drawdown deceleration + 0.20·relative strength vs ^GSPC + 0.15·depth); 6 → 4 higher-conviction slots.',
      '',
      'OUTCOME — RETIRED. The gate correctly stops buying knives, but at the 1-year pick date the biggest winners',
      '(MU, WULF, AMD, SNDK, LRCX) were STILL making new lows (negative 5-day return), so the gate excluded them',
      'too — they only turned weeks later. Capture stayed 40/150 and the 4 reserved slots displaced momentum',
      'winners → reliability 24.5, BELOW the no-sleeve v6 (25.6). This BRACKETS the ceiling: buy early = buy',
      'knives (v7, 12.9); wait for the turn = miss the not-yet-turned winners (v8, 24.5). Ray confirmed it is an',
      'INFORMATION ceiling — price/volume cannot separate a deep-drawdown winner from a loser before its catalyst',
      'appears. Reverted to v6; going further would need forward data (earnings revisions, guidance, macro).',
    ],
    results: {
      '1m': { basket:  -2.2, spx:  1.8, picks: 25, winnerHits:  5, winnerTotal: 25 }, // 1m/1d estimated
      '3m': { basket:  22.2, spx: 10.0, picks: 25, winnerHits: 11, winnerTotal: 25 },
      '6m': { basket:  15.6, spx:  8.7, picks: 25, winnerHits:  8, winnerTotal: 25 },
